...
import Language.Simple.Syntax.SExpr
import Language.Simple.Syntax.ExprBuilder
import qualified Language.Simple.Inference.Cardelli.Cardelli as C
  (infer, initialEnv, translateExpr)
import Language.Simple.Inference.SystemCT1999.Prelude
import Language.Simple.Inference.SystemCT1999.SystemCT1999
...
normal = do
  ...
  case as of
    ...
    ["--milner", s] -> do
      sexpr <- try "Parse error: " $ parseSExprs' s
      expr <- try "Wrong s-expression: " $ sexprsToExpr sexpr
      print . C.infer C.initialEnv . C.translateExpr $ expr
